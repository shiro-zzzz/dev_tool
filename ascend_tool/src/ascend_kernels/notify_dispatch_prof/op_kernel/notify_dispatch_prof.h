    ...
    int64_t scaleCount, GM_ADDR offset, int localRank, int localRankSize)
{
    ...
    // specified rank
    int coreNumPerRank = blockNum / rankSize; // Calculate the number of cores assigned to read for each rank
    int maxCore = coreNumPerRank * rankSize;  // Calculate the maximum number of cores that can be used for reading

    if (blockIdx < maxCore) {
        int readRank =
            blockIdx /
            coreNumPerRank; // Calculate the rank to be read based on the block
