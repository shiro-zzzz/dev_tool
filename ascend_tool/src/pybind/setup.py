...
# 格式: V版本.R版本.C版本.B版本
env_version = os.getenv("CAM_WHL_VERSION", "2.0.0")
...
exts = []
# Auto-collect all .cpp sources: top-level pybind dir + pytorch_extension subdir
_pybind_dir = os.path.dirname(os.path.abspath(__file__))
_sources = (
    glob.glob(os.path.join(_pybind_dir, "*.cpp")) +
    glob.glob(os.path.join(_pybind_dir, "pytorch_extension", "*.cpp"))
)
