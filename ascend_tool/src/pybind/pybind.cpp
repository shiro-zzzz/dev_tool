...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("notify_dispatch", &NotifyDispatchImpl, "notify_dispatch",
...
TORCH_LIBRARY(ascend_tool, m)
{
    ...
    m.def("notify_dispatch(Tensor send_data, Tensor token_per_expert_data, int send_count, int num_tokens, str comm_group, int rank_size, int rank_id, int local_rank_size, int local_rank_id) -> Tensor[]");
}