...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("notify_dispatch_prof", &NotifyDispatchProfImpl, "notify_dispatch_prof",
          py::arg("send_data"), py::arg("token_per_expert_data"), py::arg("prof_buf"),
...
TORCH_LIBRARY(ascend_tool, m)
{
    ...
    m.def("notify_dispatch_prof(Tensor send_data, Tensor token_per_expert_data, Tensor prof_buf, int send_count, int num_tokens, str comm_group, int rank_size, int rank_id, int local_rank_size, int local_rank_id) -> Tensor[]");
}